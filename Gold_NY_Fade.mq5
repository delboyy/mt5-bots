...
input int      RangeStartHour = 12;      // Range Start Hour (Data Time) - Gold prefers 12:30
input int      RangeStartMinute = 30;    // Range Start Minute
input int      RangeEndHour = 14;        // Range End Hour (Data Time)
      ...
      if(IsTimeAfter(dt, RangeEndHour, RangeEndMinute))
        {
         CalculateRange(time);
         rangeDefined = true;
         Print("Gold Range Defined: High=", rangeHigh, " Low=", rangeLow);
        }
     }

   // 3. Trading Logic (14:00 - 19:00)
         ...
         if(bid > rangeHigh)
           {
            ...
            if(trade.Sell(lotSize, _Symbol, bid, sl, tp, "Gold NY Fade Sell"))
              {
               tradeTaken = true;
               Print("Gold SELL Executed. Lot:", lotSize);
              }
           }
         else if(ask < rangeLow)
           {
            ...
            if(trade.Buy(lotSize, _Symbol, ask, sl, tp, "Gold NY Fade Buy"))
              {
               tradeTaken = true;
               Print("Gold BUY Executed. Lot:", lotSize);
              }
           }
        }
     }
