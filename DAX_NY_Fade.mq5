      ...
      if(IsTimeAfter(dt, RangeEndHour, RangeEndMinute))
        {
         CalculateRange(time);
         rangeDefined = true;
         Print("DAX Range Defined: High=", rangeHigh, " Low=", rangeLow);
        }
     }
         ...
         if(bid > rangeHigh)
           {
            ...
            if(trade.Sell(lotSize, _Symbol, bid, sl, tp, "DAX NY Fade Sell"))
              {
               tradeTaken = true;
               Print("DAX SELL Executed. Lot:", lotSize);
              }
           }
         else if(ask < rangeLow)
           {
            ...
            if(trade.Buy(lotSize, _Symbol, ask, sl, tp, "DAX NY Fade Buy"))
              {
               tradeTaken = true;
               Print("DAX BUY Executed. Lot:", lotSize);
              }
           }
        }
     }
